...
void rotateImage90CounterClockwise(unsigned short *imageData, int imgHeight,
                                   int imgWidth) {
  // 创建新的数组来存储旋转后的图像
...
int main0() {
  ...
  for (int i = 0; i < 600; i++) {
    ...
    // 计算平均值
    double avgVal = std::accumulate(imageData, imageData + rows * cols, 0.0) /
                    (rows * cols);
    ...
    // 双排分割线
    // for (int i = 0; i < 13; i++) {
    //     cout << "行范围: (" << result[i * 8 + 0] + minRow << ", " << result[i
    //     * 8 + 1] + minRow
    //          << "), \t列范围: (" << result[i * 8 + 2] + minCol << ", "
    //          << result[i * 8 + 3] + minCol << ")";
    //     // 输出行列宽度
    //     cout << "    \t行宽度: " << result[i * 8 + 4] << ", 列宽度: " <<
    //     result[i * 8 + 5];
    //     // 输出中心
    //     cout << "  \t中心: (" << result[i * 8 + 6] << ", " << result[i * 8 +
    //     7] << ")\n";
    // }
